...
def vn_to_basis(values, basis, massvec):
    """
    Transform data in to an orthonormal basis (where orthonormal is wrt to massvec)
    Inputs:
      - values: (B,D,3,V)
      ...
      - massvec: (B,V)
    Outputs:
      - (B,D,3,K) transformed values
    """
    basisT = basis.transpose(-2, -1).unsqueeze(1) # basisT: (B,K,V)
    BD3V = values * massvec.unsqueeze(-1).unsqueeze(-1).repeat(1,1,3,1)
    return torch.matmul(basisT, BD3V.permute(0, 2, 3, 1)).permute(0, 3, 1, 2) # ((B,1,K,V),(B,3,V,D)) -matmul-> (B,3,K,D) -permute-> (B,D,3,K)
...
def vn_from_basis(values, basis):
    """
    Transform data out of an orthonormal basis
    Inputs:
      - values: (B,D,3,K)
    ...
    """
    if values.is_complex() or basis.is_complex():
        print('complex basis is being used')
        return utils.cmatmul(utils.ensure_complex(basis), utils.ensure_complex(values))
    else:
        return torch.matmul(basis.unsqueeze(-1), values.permute(0,2,3,1)).permute(0, 3, 1, 2) # ((B,1,V,K),(B,3,K,D)) -matmul-> (B,3,V,D) -permute-> (B,D,3,V)
