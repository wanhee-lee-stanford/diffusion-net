...
from .vnn_geometry import vn_to_basis, vn_from_basis

sys.path.append(os.path.join(os.path.dirname(__file__), "../../src/"))  # add the path to the DiffusionNet src

from vnn.models.vn_layers import *
from vnn.models.utils.vn_dgcnn_util import get_graph_feature_cross
...
class LearnedTimeDiffusion(nn.Module):
    ...
    Inputs:
      - values: (V,3,C) in the spectral domain
...
class VNLearnedTimeDiffusion(nn.Module):
    """
    Applies diffusion with learned per-channel t.
    ...
    def __init__(self,
                 C_inout,
                 method='spectral'):
        super(VNLearnedTimeDiffusion,
              self).__init__()
        self.C_inout = C_inout
        self.diffusion_time = nn.Parameter(torch.Tensor(C_inout))  # (C)
        self.method = method  # one of ['spectral', 'implicit_dense']

        nn.init.constant_(self.diffusion_time,
                          0.0)
    ...
    def forward(self,
        ...
        # (and away from 0 in the incredibly rare chance that they get stuck)
        with torch.no_grad():
            self.diffusion_time.data = torch.clamp(self.diffusion_time,
                                                   min=1e-8)
        ...
        if self.method == 'spectral':
            ...
            time = self.diffusion_time
            diffusion_coefs = torch.exp(-evals.unsqueeze(-1) * time.unsqueeze(0))
            print(diffusion_coefs.shape, 'diffusion_coefs.shape')
            x_diffuse_spec = diffusion_coefs * x_spec
            print(x_diffusion_spec.shape, 'x_diffusion_spec.shape')

            # Transform back to per-vertex
            x_diffuse = vn_from_basis(x_diffuse_spec,
                                   evecs)
            ...
            # Form the dense matrices (M + tL) with dims (B,C,V,V)
            mat_dense = L.to_dense().unsqueeze(1).expand(-1,
                                                         self.C_inout,
                                                         V,
                                                         V).clone()
            mat_dense *= self.diffusion_time.unsqueeze(0).unsqueeze(-1).unsqueeze(-1)
            ...
            # Solve the system
            rhs = x * mass.unsqueeze(-1)
            rhsT = torch.transpose(rhs,
                                   1,
                                   2).unsqueeze(-1)
            sols = torch.cholesky_solve(rhsT,
                                        cholesky_factors)
            x_diffuse = torch.transpose(sols.squeeze(-1),
                                        1,
                                        2)
...
class SpatialGradientFeatures(nn.Module):
    ...
    def forward(self, vectors):
        ...
        return VNLeakyReLU(dots) # does not make sense at all
...
class DiffusionNetBlock(nn.Module):
    ...
    def __init__(self, C_width, mlp_hidden_dims,
        ...
        self.with_gradient_features = False #with_gradient_features
        self.with_gradient_rotations = False #with_gradient_rotations

        # Diffusion block
        self.diffusion = VNLearnedTimeDiffusion(self.C_width, method=diffusion_method)
        ...
        # MLPs
        self.mlp = VNMiniMLP([self.MLP_C] + self.mlp_hidden_dims + [self.C_width], dropout=self.dropout)
...
class VNDiffusionNetBlock(nn.Module):
    """
    Inputs and outputs are defined at vertices
    """
    ...
    def __init__(self,
        ...
        # Diffusion block
        self.diffusion = VNLearnedTimeDiffusion(self.C_width,
        ...
        # MLPs
        self.mlp = VNMiniMLP([self.MLP_C] + self.mlp_hidden_dims + [self.C_width],
    ...
    def forward(self,
        ...
        if self.with_gradient_features:
            ...
            for b in range(B):
                # gradient after diffusion
                x_gradX = torch.mm(gradX[b, ...],
                                   x_diffuse[b, ...])
                x_gradY = torch.mm(gradY[b, ...],
                                   x_diffuse[b, ...])

                x_grads.append(torch.stack((x_gradX, x_gradY),
                                           dim=-1))
            x_grad = torch.stack(x_grads,
                                 dim=0)
            ...
            # Stack inputs to mlp
            feature_combined = torch.cat((x_in, x_diffuse, x_grad_features),
                                         dim=-1)
        else:
            # Stack inputs to mlp
            feature_combined = torch.cat((x_in, x_diffuse),
                                         dim=-1)
...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, C_in, C_out, C_width=128, N_block=4, last_activation=None, outputs_at='vertices', mlp_hidden_dims=None, dropout=True, 
                       with_gradient_features=False, with_gradient_rotations=False, diffusion_method='spectral'):
        """
...
class VNDiffusionNet(nn.Module):
    ...
    def __init__(self,
        ...
        # Diffusion
        self.diffusion_method = diffusion_method
        if diffusion_method not in ['spectral',
                                    'implicit_dense']: raise ValueError("invalid setting for diffusion_method")
        ...
        # First and last affine layers
        self.first_lin = nn.Linear(C_in,
                                   C_width)
        self.last_lin = nn.Linear(C_width,
                                  C_out)
        ...
        for i_block in range(self.N_block):
            ...
            self.blocks.append(block)
            self.add_module("block_" + str(i_block),
                            self.blocks[-1])
    ...
    def forward(self,
        ...
        ## Check dimensions, and append batch dimension if not given
        if x_in.shape[-1] != self.C_in:
            raise ValueError("DiffusionNet was constructed with C_in={}, but x_in has last dim={}".format(self.C_in,
                                                                                                          x_in.shape[
                                                                                                              -1]))
        N = x_in.shape[-2]
        if len(x_in.shape) == 2:
        ...
        if self.outputs_at == 'vertices':
            ...
            # Remap to edges
            x_gather = x.unsqueeze(-1).expand(-1,
                                              -1,
                                              -1,
                                              2)
            edges_gather = edges.unsqueeze(2).expand(-1,
                                                     -1,
                                                     x.shape[-1],
                                                     -1)
            xe = torch.gather(x_gather,
                              1,
                              edges_gather)
            x_out = torch.mean(xe,
                               dim=-1)
            ...
            x_gather = x.unsqueeze(-1).expand(-1,
                                              ...
                                              -1,
                                              3)
            faces_gather = faces.unsqueeze(2).expand(-1,
                                                     -1,
                                                     x.shape[-1],
                                                     -1)
            xf = torch.gather(x_gather,
                              1,
                              faces_gather)
            x_out = torch.mean(xf,
                               dim=-1)
